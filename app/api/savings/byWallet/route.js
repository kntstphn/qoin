...
export async function GET(request) {
  try {
    ...
    return new Response(
      JSON.stringify({
        documents,
        message: "savings expenses by wallet and userId retrieved successfully!",
      }),
      {
        status: 200,
        headers: {
    ...
    console.error("Error fetching savings expenses by wallet and userId:", error);

    return new Response(
      JSON.stringify({
        message: "Error fetching savings expenses by wallet and userId",
      }),
      {
        status: 500,
        headers: {
