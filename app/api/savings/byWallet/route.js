...
export async function GET(request) {
  try {
    ...
    const q = query(collection(db, "savings"), where("wallet", "==", wallet));
    ...
    return new Response(
      JSON.stringify({
        totalAmount,
        message: "Savings expenses by wallet retrieved successfully!",
      }),
      {
        status: 200,
        headers: {
    ...
    console.error("Error fetching Savings expenses by wallet:", error);

    return new Response(
      JSON.stringify({
        message: "Error fetching Savings expenses by wallet",
      }),
      {
        status: 500,
        headers: {
